...
describe("CountUpValue", () => {
  it("모션 감소 환경에서는 첫 화면부터 최종 수치를 표시한다", () => {
    const { container } = render(<CountUpValue value="28억" />);

    expect(container.querySelector("[data-count-up]")).toHaveTextContent("28억");
    expect(screen.getByText("28억", { selector: ".sr-only" })).toBeInTheDocument();
  });
